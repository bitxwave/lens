...
/// INHERITS the folder's root position — it takes over the slot the
/// folder occupied, rather than being appended at the tail. This
/// matches Launchpad behavior: drag one of two cards out, the other
/// stays visually where the folder was. Idempotent — safe to call with
/// ids that no longer exist or aren't actually folders.
async fn dissolve_singleton_folders(
    ...
    for fid in candidates {
        ...
        // Fetch kind + the folder's own root slot in one round-trip.
        // Both are needed together: we skip non-folders, and if the
        // folder does dissolve, `folder_slot` is the position the lone
        // child inherits.
        let row = sqlx::query!(
            r#"SELECT kind AS "kind!: String", sort_order AS "sort_order!: i64"
               FROM cards WHERE id=?"#,
            fid
        )
        ...
        let folder_slot = row.sort_order;
        ...
        if child_count == 1 {
            // Move the lone child to the folder's root slot. We DELETE
            // the folder first (freeing the slot in the (parent_id NULL,
            // sort_order) unique index), then set the child's parent_id
            // to NULL and its sort_order to the inherited slot. A far-
            // negative staging slot bridges the two updates without ever
            // colliding with a live row.
            ...
            // 1. Park the child in staging so the folder can be deleted
            //    without the (parent_id NULL, sort_order) unique index
            //    tripping on step 3.
            sqlx::query!(
            ...
            .execute(&mut *conn)
            .await?;
            // 2. Delete the folder — this frees `folder_slot` in the
            //    root bucket.
            ...
            // 0 children — just delete the empty folder.
            sqlx::query!("DELETE FROM cards WHERE id=?", fid)
                .execute(&mut *conn)
                .await?;
        }
